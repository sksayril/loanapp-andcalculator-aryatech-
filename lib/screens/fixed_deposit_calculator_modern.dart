...
class FixedDepositCalculatorModern extends StatefulWidget {
  const FixedDepositCalculatorModern({super.key});

  @override
  State<FixedDepositCalculatorModern> createState() => _FixedDepositCalculatorModernState();
}

class _FixedDepositCalculatorModernState extends State<FixedDepositCalculatorModern> {
  double _principalAmount = 100000;
  double _interestRate = 6.5;
  int _tenureMonths = 12; // Store in months for finer control
  ...
  void _calculateFD() {
    ...
    final double tenureInYears = _tenureMonths / 12;
    
    // Calculate compound interest: A = P * (1 + r/100)^t
    final double maturityAmount = _principalAmount * pow(1 + (_interestRate / 100), tenureInYears);
    final double interestEarned = maturityAmount - _principalAmount;
  ...
  Future<void> _saveToHistory() async {
    ...
    await historyService.saveCalculation(
      calculatorType: 'fixed_deposit',
      inputData: {
        'Principal Amount': '₹${NumberFormat.currency(symbol: '', decimalDigits: 0).format(_principalAmount)}',
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
              ...
              children: [
                Expanded(
                  child: _buildInfoCard(
                    'Principal',
                    formatCurrency(_principalAmount),
                    themeProvider,
                  ),
                ),
            ...
            // FD Information Card
            _buildFdInfoCard(themeProvider),
          ],
        ),
      ),
    );
        ...
        children: [
            ...
            child: PieChart(
              PieChartData(
                ...
                sections: [
                  PieChartSectionData(
                    value: _principalAmount,
                    title: '${((_principalAmount / _maturityAmount) * 100).toStringAsFixed(1)}%',
          ...
          Row(
            ...
            children: [
              ...
              _buildLegendItem('Interest', Colors.orange.shade400, themeProvider),
            ],
          ),
        ],
      ),
  ...
  Widget _buildFdInfoCard(ThemeProvider themeProvider) {
    return Container(
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
      ...
      child: Column(
        ...
        children: [
          ...
          _buildInfoRow('• Safe and guaranteed returns'),
          _buildInfoRow('• Fixed interest rate for entire tenure'),
          _buildInfoRow('• Minimum tenure: 7 days'),
          _buildInfoRow('• Premature withdrawal available'),
